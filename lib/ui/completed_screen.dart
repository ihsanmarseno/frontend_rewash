...
class CompletedScreen extends ConsumerWidget {
  ...
  Widget _buildTaskList(BuildContext context, WidgetRef ref, List<Task> tasks) {
    ...
    return Column(
      children: [
        _buildCompletedTasksCard(context, completedTaskCount),
        Expanded(
          child: ListView.builder(
            itemCount: completedTasks.length,
            itemBuilder: (context, index) =>
                _buildTaskCard(context, ref, completedTasks[index]),
          ),
        ),
      ],
    );
  ...
  Widget _buildCompletedTasksCard(BuildContext context, int count) {
    return Container(
      ...
      child: ListTile(
        ...
        trailing: PopupMenuButton<String>(
          onSelected: (value) async {
            switch (value) {
              case 'pending':
                await _markTaskAsPending(context, ref, task);
                break;
              case 'edit':
          ...
          itemBuilder: (BuildContext context) => [
            _buildPopupMenuItem('Pending', Icons.undo, Colors.redAccent),
  ...
  Future<void> _markTaskAsPending(
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to mark task as pending: ${e.toString()}'),
